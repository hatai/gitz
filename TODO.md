...
#### phase 3b（残り・未実装）
- [ ] 日付範囲（`--since`/`--until`・UTC 基準・timezone/境界包含の仕様化・spec §16/M9）
- [ ] パス（`-- <path>`・pathspec semantics の定義・literal or glob・spec §16/MINOR3）
- [ ] ブランチ（`--branches`・`<snapshot_tip>` との和集合問題の解決が前提・単一 branch は hash 解決して snapshot_tip へ・複数 branch は所有集合・spec §16/B3）
- [ ] フィルタ中の graph 維持（`graph.zig` の nearest-visible-parent 投影 or Git history simplification・別 spec・M1/M2/B2・phase3a は `graph_render_policy=.suppressed` で非表示で回避）
- [ ] StreamTooLong の limit 注入 seam（テスト容易化・`git/process.zig`・spec §6.3・phase3a は catch で LogLoadFailed/LogPageFailed へ正規化のみ）
- [ ] busy lifecycle 完全修正（runtime lifecycle（main の `reapWorker`/`dispatchSideEffect`）のみで busy を管理・reducer で busy を触らない・M-N9・phase3a は log 中 git_error 無視で最小対処）

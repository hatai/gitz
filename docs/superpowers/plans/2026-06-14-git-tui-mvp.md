...
- `src/update.zig` — 純粋 reducer `update(model: *Model, msg: Msg) !AppCmd`（Model を破壊的更新し AppCmd を返す）。zigzag 非依存。
...
pub const Model = struct {
    ...
    pub fn replaceFiles(self: *Model, entries: []const status.StatusEntry) !void {
        ...
        errdefer { for (next.items) |*f| { a.free(f.path); if (f.orig_path) |p| a.free(p); } next.deinit(a); }
        for (entries) |e| {
...
pub fn update(model: *Model, msg: Msg) !AppCmd {
    switch (msg) {
        ...
        .select_index => |i| { if (i < model.files.items.len) model.selected = i; model.focus = .changes; model.diff_scroll = 0; return loadDiffCmd(model); },
        ...
        .set_focus => |f| { model.focus = f; return .none; },
        .toggle_stage => {
        ...
        .commit_text_changed => |text| { try model.setStr(&model.commit_message, text); return .none; },
        .quit => return .quit,
        // 解釈器からの結果
...
const TmpRepo = struct {
    ...
    fn git(self: *TmpRepo, a: std.mem.Allocator, io: std.Io, argv: []const []const u8) !void {
        var r = try process.run(a, io, argv, self.cwd());
        r.deinit(a);
    }
...
test "repoRoot resolves from a subdirectory cwd" {
    ...
    var repo = try TmpRepo.init(a, io);
    defer repo.deinit();
    try repo.dir.dir.makePath("sub/deep");
    var sub = try repo.dir.dir.openDir("sub/deep", .{});
    defer sub.close();
...
pub fn mouseToMsg(ev: MouseEvent) ?Msg {
    return switch (ev.kind) {
        ...
        .left_click => if (ev.file_row) |r| .{ .select_index = r }
            else if (ev.pane) |p| .{ .set_focus = p } else null,
...
`fromZigzagMouse(ev, layout, click_state) MouseEvent` を実装する。Ctrl 修飾キーは zigzag のキー修飾子から
`ctrl_s`/`ctrl_d`/`ctrl_u` にマップ。マウスは:
- **ペイン判定**: クリック座標と `view.computeLayout` の各 `Rect` を照合して `pane`（changes/diff/commit）と、
  changes ペインなら行→`file_row` を算出。`on_diff` は diff ペイン上か。
- **ダブルクリック判定はこのアダプタ層が担う**（reducer は時刻を持てない＝純粋）。直近クリックの
  「時刻＋座標」を保持する小さな `ClickState` を持ち、しきい値（例: 300ms）以内かつ同一行なら
  `left_double`、そうでなければ `left_click` を生成する。時刻は zigzag のイベントタイムスタンプ、
  無ければ `std.time.milliTimestamp()` を使う。
- **commit フォーカス時**: 文字・改行・カーソル・Backspace の各キーは `Key` に変換せず（または変換しても
  `keyToMsg` が null を返す）、**zigzag `TextArea` の update API へ直接渡す**（編集は TextArea が正本）。

この部分は zigzag 依存のため `test` は付けず、Task 11 のヘッドレス/手動検証でカバーする。

```zig
// 例（実際の zigzag 型名は api-notes に合わせる）:
// pub const ClickState = struct { last_ms: i64 = 0, last_row: ?usize = null };
// pub fn fromZigzagKey(ev: zz.Key) ?Key { ... }
// pub fn fromZigzagMouse(ev: zz.Mouse, layout: view.Layout, cs: *ClickState) MouseEvent { ... }
```
...
- Changes ペイン: Staged / Unstaged / Untracked のセクション見出しと各ファイル行。`selected` を反転表示。
  rename は `old → new` 形式で表示。
- Diff ペイン: `model.diff_text` を **`model.diff_scroll` を先頭行として**描画。`+`/`-` を色分け。
- Commit ペイン: **zigzag `TextArea` が編集の正本**（多行・カーソル・多バイトは TextArea が処理）。
  `focus==.commit` のときキー入力を TextArea へ送り、**TextArea の内容が変わったら
  `Msg.commit_text_changed = textarea.text()` を発行**して Model にキャッシュ同期する。
  `request_commit`（Ctrl+S）時に Model.commit_message が使われる。`focus==.commit` で枠を強調。
- Status バー: `model.branch`、`busy` ならスピナ、`error_text` があれば表示、キーヒント。
- **マウス**: `model.mouse_enabled` が true のときのみ zigzag のマウストラッキングを有効化する。
...
0. `std.Io.Threaded` を構築して `io` を得る（git 実行系に渡す。正確な構築は api-notes 参照）。
   起動引数に `--no-mouse` があれば後で `model.mouse_enabled = false` にする（spec §6 オプトアウト）。
1. `cmds.repoRoot(allocator, io, .inherit)` を取得。null ならエラー表示して終了（spec §8）。
...
| 5. 日本語ファイル名・メッセージ | Task 3（utf8 パス）+ Task 10（TextArea 編集+幅）+ Task 7（commit_text_changed 同期）|

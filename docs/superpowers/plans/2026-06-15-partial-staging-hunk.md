...
`src/update.zig` の `.key_down` / `.key_up` / `.select_index` の 3 アームに、既存の
`model.diff_scroll = 0;` の直後（`return loadDiffCmd(model);` の前）へ `model.selected_hunk = 0;` を
追加する。3 アームとも行構成が微妙に異なるので、置換後の全文を明示する:
...
`README.md` の `## 操作キー` は markdown テーブル（`| キー | 動作 |`）。diff ペインフォーカス時の
挙動を行として追記する。`| q | 終了 |` 行（README:59）の直後に以下の 3 行を挿入する:
...
さらにテーブル下部のマウス箇条書き（README:65-68）の末尾に 2 行追加する:

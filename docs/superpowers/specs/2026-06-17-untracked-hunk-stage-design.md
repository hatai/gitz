...
### 4.1 純粋層テスト（`src/diff/hunk.zig` 内・新規 1 件のみ）
...
3. **untracked のハンク stage**: 1 ファイル内に複数ハンク相当の変更がある場合でも、1 ハンク（`@@ ... @@`
   単位）の一部を行選択して stage できる（untracked は `--no-index` で単一ハンクになることが多いが、
   理論上は複数ハンクも可能）。
...
7. **エラーを握り潰さない**: 不正なパッチで `git apply --cached` が exit!=0 のとき、既存の `git_error`
   ...
   テストが担保）。
8. **No-newline 境界は安全 no-op**: untracked で末尾改行無しのファイルの最終行のみを選択した場合、
   `buildLinePatch` の既存 no-newline 矛盾判定が発火して `null`（no-op）になり、矛盾パッチを git に渡さない。
   reducer が既存のガイダンス「選択範囲を stage できません（変更行なし、または末尾改行境界）」を出す。
9. **busy 中の操作は直列化**: untracked の部分 stage 実行中にもう一度 `s` を押しても、`model.busy` ゲートと
   main.zig の `pending` latest-wins で直列化される（既存の全副作用と同一経路）。

## 6. 実装順（純粋層 TDD → 結合）

1. **`src/diff/hunk.zig`**: untracked 形式入力の `buildLinePatch` テスト 1 件を追加（実装は変更不要・
   テストのみ）。`zig build test` で green を確認。
2. **`src/update.zig`**:

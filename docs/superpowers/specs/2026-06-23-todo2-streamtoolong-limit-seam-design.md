...
- **`runWithLimit` が小 limit 超過で StreamTooLong**: `runWithLimit(a, io, &.{"echo", "hello"}, .inherit, .limited(2))`
  ...
  - これが seam の前提（limit 超過 = truncate ではなく error）を**直接実証**するテスト。
- **`run` 既定動作の回帰**: 既存 2 テスト（echo / false）はそのまま通る（`run` がラッパ化されても挙動同一）。

### 4.2 appcmd.zig（結合・TmpRepo）

- **`runLogInt` StreamTooLong → `log_load_failed`**: `TmpRepo` に 1 コミット作成 →
  ...
  - cmd ペイロード（`AppCmd.LoadLog`）の所有権はテスト側で管理（既存 appcmd テストの構築パターンに合わせる）。
...
- ⚠️ **要実証（実装の最初）**: `std.process.run` が `stdout_limit` 超過時に truncate ではなく `error.StreamTooLong` を**返す**こと。
  §4.1 の process.zig 単体テストが最初に通れば前提成立。万一 truncate 挙動だった場合は seam 自体が成立しないため、
  その時点で `update`/`appcmd` の正規化方式（出力長を自前チェック等）を再検討（spec へ戻す）。
  - phase 3a spec §6.3 / process.zig:12 のコメントは StreamTooLong が RunError に含まれる前提だが、未実証。
